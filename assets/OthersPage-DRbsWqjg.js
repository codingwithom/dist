import { c as createLucideIcon, i as useAppContext, H as idbGet, J as idbSet, j as jsxRuntimeExports, B as Button, z as FileText, a0 as Layers, p as ChevronDown, Z as BookOpen, d as CalendarDays, aB as GraduationCap, h as ChevronRight, y as ArrowLeft, S as Search, I as Input, v as LoaderCircle, o as Play, f as Check, a5 as Eye, g as ChevronLeft, X, F as Flame, ar as Sparkles, aC as useLocation, C as Card, af as Bookmark, $ as BrainCircuit } from "./index-CrNMMc0h.js";
import { b as reactExports } from "./vendor-charts-Du7SI2xx.js";
import { i as isCbtEligible, Q as QuestionsPage, N as NtaCbtExamSimulator, F as FileQuestionMark } from "./QuestionsPage-BR0O-05U.js";
import { B as Badge, E as ExternalLink } from "./badge-Dk171B7X.js";
import { R as RefreshCw } from "./refresh-cw-CneX1rC4.js";
import { F as FolderOpen } from "./folder-open-DekRSs-o.js";
import { C as Clock } from "./clock-DqOm1iE5.js";
import { D as Download } from "./download-De283x-O.js";
import { A as ArrowRight } from "./info-oQ5epHa7.js";
import "./vendor-firebase-C_hAKoH5.js";
import "./vendor-math-DSzbeUdl.js";
import "./rotate-ccw-Bb6Bxmkq.js";
import "./zap-tWbyY5pN.js";
import "./trash-2-BYOFUZgO.js";
import "./ellipsis-vertical-DTgSUBQi.js";
import "./target-DJMNRald.js";
import "./circle-check-C_AyBa8j.js";
import "./circle-x-D7YnVgKF.js";
const __iconNode$1 = [
...
function mergeBatches(existing, fresh) {
  ...
  const mergedSubjects = fresh.subjects.map((freshSub) => {
    ...
    const mergedChapters = freshSub.chapters && freshSub.chapters.length > 0 ? freshSub.chapters.map((freshCh) => {
  ...
  try {
    ...
    for (let i = 0; i < pages.length; i += 4) {
      ...
      const chunkResults = await Promise.all(
        chunk.map(
          (page) => fetch(`${origin}/api/v2/batches/${encodeURIComponent(batchId)}/weekly-schedules?batchId=${encodeURIComponent(batchId)}&startDate=${sDate}&endDate=${eDate}&page=${page}`, {
            headers,
            signal: AbortSignal.timeout(8e3)
          }).then((r) => r.ok ? r.json() : null).catch(() => null)
        )
...
function PWPage() {
  ...
  reactExports.useEffect(() => {
    ...
    fetch(`/api/pw-schedule?batchId=${encodeURIComponent(selectedBatchId)}&date=${encodeURIComponent(todayIstDate)}&month=${encodeURIComponent(calendarMonthKey)}`, { cache: "no-store" }).then((res) => res.ok ? res.json() : null).then(async (payload) => {
      ...
      const isSynth = !Array.isArray(allSchedules) || allSchedules.length === 0 || allSchedules.every((s) => String(s.id || "").startsWith("synth-"));
      if (isSynth) {
        const direct = await fetchDirectBatchSchedule(selectedBatchId, calendarMonthKey).catch(() => null);
        if (direct && Array.isArray(direct.allSchedules) && direct.allSchedules.length > 0 && !direct.allSchedules.every((s) => String(s.id || "").startsWith("synth-"))) {
      ...
      try {
        ...
        if (data && Array.isArray(data.lectures)) {
          const updatedCh = {
            ...
            videoCount: data.totalLectures !== void 0 && data.totalLectures > 0 ? data.totalLectures : data.lectures.length || ch.videoCount,
            dppCount: data.totalDpps !== void 0 && data.totalDpps > 0 ? data.totalDpps : data.dpps?.length || ch.dppCount,
            notesCount: data.totalNotes !== void 0 && data.totalNotes > 0 ? data.totalNotes : data.notes?.length || ch.notesCount,
  ...
  const latestActiveScheduleDate = reactExports.useMemo(() => {
  ...
  const currentWeekDays = reactExports.useMemo(() => {
    const base = /* @__PURE__ */ new Date((selectedScheduleDate || todayIstDate) + "T00:00:00Z");
      ...
      activeTab === "schedule" && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-8", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { id: "weekly-schedule", className: "pt-4 border-t border-slate-200/80 dark:border-zinc-800", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "grid grid-cols-1 lg:grid-cols-12 gap-6 items-start", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lg:col-span-8 space-y-4", children: [
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none", children: currentWeekDays.map((d) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                onClick: () => setSelectedScheduleDate(d.dateStr),
                className: `flex flex-col items-center justify-center min-w-[56px] sm:min-w-[62px] py-2 px-2.5 rounded-xl text-center border transition-all cursor-pointer ${d.isSelected ? "bg-amber-500 text-white border-amber-500 shadow-xs font-bold" : d.isToday ? "bg-amber-50 dark:bg-amber-950/30 border-amber-300 dark:border-amber-700 text-amber-700 dark:text-amber-300 font-semibold" : "bg-white dark:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-700 dark:text-zinc-300 hover:border-slate-300 dark:hover:border-zinc-700"}`,
                children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[10px] uppercase tracking-wider font-semibold opacity-80", children: d.dayName }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-sm sm:text-base font-extrabold", children: d.dayNum }),
                  d.hasClasses ? /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: `text-[10px] mt-0.5 px-1.5 py-0.2 rounded-full font-medium ${d.isSelected ? "bg-white/20 text-white" : "bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300"}`, children: [
                    d.classCount,
                    " ",
                    d.classCount === 1 ? "class" : "classes"
                  ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "text-[9px] mt-0.5 text-slate-400 dark:text-zinc-500", children: "Off" })
                ]
              },
              d.dateStr
            )) }),
            ...
            ) : groupedSchedule.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "p-8 sm:p-10 text-center rounded-2xl border border-dashed border-slate-300 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 bg-white dark:bg-zinc-900/30 space-y-3", children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-12 h-12 rounded-full bg-amber-50 dark:bg-amber-950/40 text-amber-500 flex items-center justify-center mx-auto text-xl", children: "📅" }),
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsxs("h4", { className: "font-bold text-slate-800 dark:text-zinc-200", children: [
                  "No Classes Scheduled for ",
                  selectedScheduleDate
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("p", { className: "text-xs text-slate-500 dark:text-zinc-400 mt-1", children: [
                  "This is a rest or self-study day for ",
                  currentBatch.name,
                  "."
                ] })
              ] }),
              latestActiveScheduleDate && latestActiveScheduleDate !== selectedScheduleDate && /* @__PURE__ */ jsxRuntimeExports.jsx(
                Button,
                {
                  variant: "outline",
                  size: "sm",
