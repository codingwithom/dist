import { c as createLucideIcon, i as useAppContext, H as idbGet, J as idbSet, j as jsxRuntimeExports, B as Button, z as FileText, a0 as Layers, p as ChevronDown, Z as BookOpen, d as CalendarDays, aA as GraduationCap, h as ChevronRight, y as ArrowLeft, S as Search, I as Input, v as LoaderCircle, o as Play, f as Check, a5 as Eye, g as ChevronLeft, X, F as Flame, aq as Sparkles, aB as useLocation, C as Card, ae as Bookmark, $ as BrainCircuit } from "./index-Djut9CSo.js";
import { b as reactExports } from "./vendor-charts-Du7SI2xx.js";
import { i as isCbtEligible, Q as QuestionsPage, N as NtaCbtExamSimulator, F as FileQuestionMark } from "./QuestionsPage-Coe2qVrS.js";
import { B as Badge, E as ExternalLink } from "./badge-BzjLcBHC.js";
import { R as RefreshCw } from "./refresh-cw-cd0jb2UG.js";
import { F as FolderOpen } from "./folder-open-DBw96JDv.js";
import { C as Clock } from "./clock-DodAe5k8.js";
import { D as Download } from "./download-RfepGwxo.js";
import { A as ArrowRight } from "./info-CNlvAZgE.js";
import "./vendor-firebase-C_hAKoH5.js";
import "./vendor-math-DSzbeUdl.js";
import "./target-DluCw5-L.js";
import "./rotate-ccw-DfeJoWoc.js";
import "./zap-DLg7JjLC.js";
import "./trash-2-C0Ji0Pd7.js";
import "./ellipsis-vertical-D4E_64Qe.js";
import "./circle-check-SyBZIxgU.js";
import "./circle-x-D7D6sPfH.js";
const __iconNode$1 = [
...
function cleanBatchDescription(desc) {
  ...
  let text = desc.replace(/<style[\s\S]*?<\/style>/gi, " ").replace(/<script[\s\S]*?<\/script>/gi, " ").replace(/<[^>]+>/g, " ");
  text = text.replace(/\{[^}]*\}/g, " ").replace(/\.[a-zA-Z0-9_-]+\s*\{[^}]*\}/g, " ");
...
async function fetchDirectChapterContents(batchId, subjectId, chapterId) {
  ...
  const [vRes, nRes, dRes] = await Promise.all([
    fetch(`${origin}/api/v2/batches/${encodeURIComponent(batchId)}/subject/${encodeURIComponent(subjectId)}/contents?page=1&contentType=videos&tag=${encodeURIComponent(chapterId)}`, { headers, signal: AbortSignal.timeout(9e3) }).then((r) => r.ok ? r.json() : { data: [] }).catch(() => ({ data: [] })),
    fetch(`${origin}/api/v2/batches/${encodeURIComponent(batchId)}/subject/${encodeURIComponent(subjectId)}/contents?page=1&contentType=notes&tag=${encodeURIComponent(chapterId)}`, { headers, signal: AbortSignal.timeout(9e3) }).then((r) => r.ok ? r.json() : { data: [] }).catch(() => ({ data: [] })),
    fetch(`${origin}/api/v2/batches/${encodeURIComponent(batchId)}/subject/${encodeURIComponent(subjectId)}/contents?page=1&contentType=DppNotes&tag=${encodeURIComponent(chapterId)}`, { headers, signal: AbortSignal.timeout(9e3) }).then((r) => r.ok ? r.json() : { data: [] }).catch(() => ({ data: [] }))
  ]);
