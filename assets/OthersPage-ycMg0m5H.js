import { c as createLucideIcon, i as useAppContext, H as idbGet, J as idbSet, j as jsxRuntimeExports, B as Button, z as FileText, a0 as Layers, p as ChevronDown, Z as BookOpen, d as CalendarDays, aA as GraduationCap, h as ChevronRight, y as ArrowLeft, S as Search, I as Input, v as LoaderCircle, o as Play, f as Check, a5 as Eye, g as ChevronLeft, X, F as Flame, aq as Sparkles, aB as useLocation, C as Card, ae as Bookmark, $ as BrainCircuit } from "./index-Czn-ZwzM.js";
import { b as reactExports } from "./vendor-charts-Du7SI2xx.js";
import { i as isCbtEligible, Q as QuestionsPage, N as NtaCbtExamSimulator, F as FileQuestionMark } from "./QuestionsPage-CKI33bHH.js";
import { B as Badge, E as ExternalLink } from "./badge-BPMBuGD-.js";
import { R as RefreshCw } from "./refresh-cw-BRxJC6yJ.js";
import { F as FolderOpen } from "./folder-open-056jZmwB.js";
import { C as Clock } from "./clock-DKJOVXGJ.js";
import { D as Download } from "./download-BuN35Zm3.js";
import { A as ArrowRight } from "./info-B1QDMB1_.js";
import "./vendor-firebase-C_hAKoH5.js";
import "./vendor-math-WRPH5VQa.js";
import "./target-ByGW57ri.js";
import "./rotate-ccw-opsWwN__.js";
import "./zap-DuFHB-V5.js";
import "./trash-2-BF6sy8Wv.js";
import "./ellipsis-vertical-C31LGkD2.js";
import "./circle-check-BHC88q9y.js";
import "./circle-x-DiVu2vj1.js";
const __iconNode$1 = [
...
function cleanBatchDescription(desc) {
  ...
  let text = desc.replace(/<style\b[^<]*(?:(?!<\/style>)<[^<]*)*<\/style>/gi, " ").replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, " ").replace(/<[^>]+>/g, " ");
  ...
  (ch.lectures || []).forEach((item, idx) => {
    ...
    if (item.allDpps && item.allDpps.length > 0) {
      item.allDpps.forEach((dp, dIdx) => {
        const dppKey = dp?.pdf || `${item.id}-dpp-${dIdx}`;
      ...
      const dppKey = pdf || `${item.id}-dpp-direct`;
      if (!seenDpps.has(dppKey)) {
        seenDpps.add(dppKey);
        dpps.push({
...
function PWPage() {
  ...
  reactExports.useEffect(() => {
    fetch("/api/pw-catalog", { cache: "no-store" }).then((res) => res.ok ? res.json() : null).then((data) => {
  ...
  reactExports.useEffect(() => {
    if (!selectedBatchId) return;
    fetch(`/api/pw-schedule?batchId=${encodeURIComponent(selectedBatchId)}&date=${encodeURIComponent(todayIstDate)}&month=${encodeURIComponent(calendarMonthKey)}`, { cache: "no-store" }).then((res) => res.ok ? res.json() : null).then((payload) => {
      if (payload) {
        if (Array.isArray(payload.schedules)) setTodaySchedule(payload.schedules);
        if (Array.isArray(payload.allSchedules)) {
          setAllBatchSchedules((prev) => {
            const map = /* @__PURE__ */ new Map();
            prev.forEach((item) => map.set(item.id, item));
            payload.allSchedules.forEach((item) => map.set(item.id, item));
        ...
        if (Array.isArray(payload.availableDates)) {
          setAvailableScheduleDates((prev) => Array.from(/* @__PURE__ */ new Set([...prev, ...payload.availableDates])).sort());
        }
      }
    }).catch(() => {
    });
  ...
  reactExports.useEffect(() => {
    ...
    fetch(`/api/pw-schedule?batchId=${encodeURIComponent(selectedBatchId)}&date=${encodeURIComponent(selectedScheduleDate)}&month=${encodeURIComponent(calendarMonthKey)}`, { cache: "no-store" }).then((res) => res.ok ? res.json() : null).then((payload) => {
      if (payload) {
        if (Array.isArray(payload.schedules) && payload.schedules.length > 0) {
          setDateSchedule(payload.schedules);
        } else {
          const localMatch = allBatchSchedules.filter((s) => s.date === selectedScheduleDate);
          setDateSchedule(localMatch.length > 0 ? localMatch : payload.schedules || []);
        }
        if (Array.isArray(payload.allSchedules)) {
          setAllBatchSchedules((prev) => {
            const map = /* @__PURE__ */ new Map();
  ...
  const filteredCatalog = reactExports.useMemo(() => {
    ...
    return catalogBatches.filter((b) => {
      return b.name.toLowerCase().includes(q) || b.exam && b.exam.toLowerCase().includes(q) || b.class && b.class.toLowerCase().includes(q);
    }).slice(0, 48);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-h-screen bg-slate-50 dark:bg-zinc-950 text-slate-900 dark:text-zinc-100 transition-colors pb-24", children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("main", { className: "max-w-7xl mx-auto px-4 sm:px-6 pt-5 pb-16", children: [
      activeTab === "curriculum" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "space-y-5", children: !selectedSubject ? /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-5", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "space-y-5", children: [
          ...
          subjectSubTab === "books" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3.5", children: filteredDigitalBooks.length === 0 ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "col-span-full p-12 text-center rounded-2xl border border-dashed border-slate-300 dark:border-zinc-800 text-slate-500 dark:text-zinc-400 bg-white dark:bg-zinc-900/30", children: activeSubjectCategorized.digitalBooks.length === 0 ? "Digital books and modules for this subject are available in the batch resources tab." : `No digital books found matching "${chapterSearchQuery}".` }) : filteredDigitalBooks.map((ch, idx) => {
            const cleanTitle = cleanChapterTitle(ch.title);
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "div",
              {
                  ...
                  /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center justify-between gap-3", children: [
                    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex items-center gap-3.5 min-w-0 flex-1", children: [
                      isDpp ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-11 h-11 rounded-xl bg-indigo-50 dark:bg-indigo-950/40 border border-indigo-200 dark:border-indigo-800/60 flex items-center justify-center text-indigo-600 dark:text-indigo-400 shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(FileDown, { className: "w-5 h-5" }) }) : isNotes ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-11 h-11 rounded-xl bg-sky-50 dark:bg-sky-950/40 border border-sky-200 dark:border-sky-800/60 flex items-center justify-center text-sky-600 dark:text-sky-400 shrink-0", children: /* @__PURE__ */ jsxRuntimeExports.jsx(FileText, { className: "w-5 h-5" }) }) : /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-11 h-11 rounded-xl overflow-hidden relative border border-slate-200 dark:border-zinc-700 bg-slate-100 dark:bg-zinc-800 shrink-0", children: [
                        teacherImg ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: teacherImg, alt: item.teacher, className: "w-full h-full object-cover" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "w-full h-full flex items-center justify-center font-bold text-amber-600 dark:text-amber-400 text-sm", children: item.teacher?.charAt(0) || "P" }),
    ...
    batchModalOpen && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl w-full max-w-xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "flex-1 overflow-y-auto p-4 space-y-2", children: filteredCatalog.map((b) => {
        ...
        return /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "div",
          {
            ...
            className: `p-3.5 rounded-xl border transition-all cursor-pointer flex items-center justify-between gap-3 ${isCurrent ? "bg-amber-500/10 border-amber-500/40 text-amber-900 dark:text-amber-200" : "bg-white dark:bg-zinc-900/60 hover:bg-slate-50 dark:hover:bg-zinc-900 border-slate-200 dark:border-zinc-800 text-slate-800 dark:text-zinc-200"}`,
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "min-w-0 flex-1", children: [
              ...
              isCurrent ? /* @__PURE__ */ jsxRuntimeExports.jsx(Badge, { className: "bg-amber-500 text-white text-[10px] font-bold", children: "Active" }) : /* @__PURE__ */ jsxRuntimeExports.jsx(ChevronRight, { className: "w-4 h-4 text-slate-400" })
            ]
          },
          b.batch_id
        );
    ...
    (!currentBatch.subjects || currentBatch.subjects.length === 0) && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 backdrop-blur-xl bg-slate-900/40 dark:bg-black/65 transition-all duration-300 animate-in fade-in", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative w-full max-w-md rounded-3xl border border-amber-500/30 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-2xl p-6 sm:p-8 shadow-2xl overflow-hidden", children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "relative z-10 flex flex-col items-center text-center", children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "w-full space-y-2", children: [
          ...
          catalogBatches.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx(
            Button,
            {
              variant: "outline",
              onClick: () => setBatchModalOpen(true),
